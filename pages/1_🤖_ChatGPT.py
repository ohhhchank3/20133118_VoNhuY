...
def render_sidebar_gpt_config_tab(zone):
   ...
   st.session_state["params"]["apikey_hello"] = zone.text_input(
        "Nhập mã API key",
        value='***' if real_psid12 else "None",
        key="input_psid12",
...
def render_sidebar():
    ...
    tab_gpt,tab_rapid,tab_prompthuggingface = chat_config_expander.tabs(
        ['🌐  ChatBot','👥 Hộp thoại gợi ý','❄️ Rapid API',"🤗 Hugging Face"]
    )
    ...
    if st.session_state["params"]["model"] in {'chatbot-fake'}:
     ...
     with st.spinner("Chờ phản hồi..."):
        ...
        if st.session_state["params"]["model_openai"] in {'Use_API_Key'}:
         with st.spinner("Chờ phản hồi..."):
          apikey = st.session_state["params"]["apikey_hello"]
